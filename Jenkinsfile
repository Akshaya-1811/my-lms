pipeline {
    ...
    stages {
        stage('sonar-analysis') {
            steps {
                echo 'analyzing..'
                sh 'cd webapp && sudo docker run  --rm -e SONAR_HOST_URL="http://18.236.78.16:9000" -e SONAR_LOGIN="sqp_f8a41db6eaa399d8aed2323273281891bc6bade8"  -v ".:/usr/src" sonarsource/sonar-scanner-cli -Dsonar.projectKey=lms'
            }
        }
        stage('buildt') {
            steps {
